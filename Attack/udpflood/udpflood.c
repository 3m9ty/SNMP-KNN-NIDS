...
int main(int argc, char *argv[])
{
  ...
  payload_generate(atoi(argv[3]));
  //printf("content %s packet %d\n",payload,atoi(argv[3]));
  strcpy(data, payload);
...
void payload_generate(int p)
{
  char seed_str[] = "qwertyuiopasdfghjklzxcvbnm"
  "QWERTYUIOPASDFGHJKLZXCVBNM1234567890";
  
  int seed_str_len = strlen(seed_str);
  for(int u = 0;u < p; u++)
  {
    int random_num = rand()%seed_str_len;
    payload[u] = seed_str[random_num];
  }
}
